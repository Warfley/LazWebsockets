...
uses
  Classes, SysUtils, gvector, fgl, {$IfDef WINDOWS}windows{$EndIf};
...
type    
  ...
  public
    procedure Init;
    procedure Done;
    function Lock(timeout: Integer=-1): Boolean;
    procedure Unlock;
  end;
...
function TMutex.Lock(timeout: Integer): Boolean;
begin
  {$IfDef WINDOWS}
  Result := WaitForSingleObject(semaphore, timeout) = WAIT_OBJECT_0;
  {$EndIf}
end;
